...
#GROUP_COLORS     <- c("treatment" = "#118ACB", "control" = "#E54B4B")

MANUAL_BREAKS <- list(
  milex_gdp = seq(0, 3.2, by = 0.8),
  pop       = seq(0, 140, by = 35),
...
saveRDS(master_panel_log, file = MASTER_PANEL_LOG)

message(paste("\n--- Script 02_eda.R finished ---",
              "\nAll output (tables and figures) saved to:", here::here("_output"),
              "\nLogged master panel data frame saved to: ", MASTER_PANEL_LOG))
