...
library(ggfortify) # Diagnostikplots
...
# Output filstier
DIR_TAB <- here("_output", "_tables", "_ols_prelims")
DIR_FIG <- here("_output", "_figures", "_ols_prelims")
...
run_model_diagnostics <- function(data, model_label, config) {
  ...
  # Generer plots (Side Effect)
  file_slug <- config$outcome
  # Standard diagnostik plot
  p_diag <- autoplot(
    model,
    which = 1:4,
    label.size = 3,
...
# Gem resultaterne fra testene i en samlet tabel
diag_table <- diag_results %>%
  ...
  fmt(columns = c(Shapiro_P, BP_P, RESET_P), fns = function(x) style_pvalue(x, digits = 3)) %>%
  fmt_number(columns = MAX_VIF, decimals = 3, dec_mark = ",", sep_mark = ".") %>%
  ba_theme_gt() %>%
