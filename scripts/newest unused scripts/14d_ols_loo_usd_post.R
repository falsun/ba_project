...
formula_full <- milex_usd_post ~ dist_enemy_log + border_rus + nato_gap_2021 + debt_gdp_2021_log
...
jackknife_results <- map_dfr(countries, function(dropped_country) {
  ...
  tidy_res %>%
    filter(term %in% c("dist_enemy_log", "border_rus", "nato_gap_2021", "debt_gdp_2021_log")) %>%
...
true_coeffs <- tidy(model_full) %>%
  ...
  mutate(
    term_label = case_when(
      ...
      term == "nato_gap_2021" ~ "Afstand til 2% mål (procentpoint)",
      term == "debt_gdp_2021_log" ~ "Log offentlig gæld (% BNP)",
      TRUE ~ term
    )
  )
...
# Save
ggsave(here("_output", "_figures", "ols_loo_usd_post.png"), p_jack, width = 8, height = 8, bg = "white")
# ---------------------------------------------------------------------------- #
