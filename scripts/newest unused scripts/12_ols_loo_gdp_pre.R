...
formula_full <- milex_gdp_pre ~ dist_enemy_log + border_rus + nato_gap_2014 + debt_gdp_2014_log
...
jackknife_results <- map_dfr(countries, function(dropped_country) {
  ...
  tidy_res %>%
    filter(term %in% c("dist_enemy_log", "border_rus", "nato_gap_2014", "gdp_2014_log")) %>%
...
# Save
ggsave(here("_output", "_figures", "ols_loo_gdp_pre.png"), p_jack, width = 8, height = 8, bg = "white")
# ---------------------------------------------------------------------------- #
