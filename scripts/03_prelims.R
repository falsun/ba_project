...
# --- Variables to Test ---
VARS_TO_TEST <- c(
  "milex_cap", "milex_cap_log",
  "milex_gdp", "milex_gdp_log",
  "milex_usd_log"
)
...
DIR_TAB     <- here::here("_output", "_tables", "_prelims")
...
# A. Create Analysis Dataframe (Full Timeline for Event Study)
...
csd_comparison_results <- map_dfr(VARS_TO_TEST, function(var_name) {
  ...
  # -- 3. Test Main Model --
  form_main <- as.formula(glue("{var_name} ~ 1 | iso3c + year + treat_dummy[year]"))
