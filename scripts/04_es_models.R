  ...
  for (spec_name in names(model_specs)) {
    ...
    tryCatch({
      ...
      plot_es <- ggplot(table_data, aes(x = event_time_num, y = estimate)) +
        ...
        # Error Bars & Points
        geom_errorbar(aes(ymin = uniform.low, ymax = uniform.high), width = 0, color = "black") +
...
for (var_name in VARS_FOR_ANALYSIS) {
  ...
  # --- B. TREND CORRECTION TABLE (CONSOLIDATED) ---

  message("   Generating Consolidated Trend Correction Table...")

  # 1. Calculate Corrections for ALL variables
  correction_data <- map_dfr(VARS_FOR_ANALYSIS, function(v_code) {

    # Run Model to get Slope
    f_main <- as.formula(glue::glue("{v_code} ~ i(event_time, treat_dummy, ref = c(-1, -8)) + i(treat_dummy, year, ref=0) | iso3c + year"))
    mod_main <- feols(f_main, data = es_panel, cluster = ~iso3c)
    slope <- coef(mod_main)[["treat_dummy::1:year"]]

    # Create Tibble for this variable
    tibble(
      year = 2014:2024,
      var_label = VAR_LABELS[v_code], # Use the nice Danish name
      slope = slope, # Keep this for the footer later
      correction = slope * (year - 2014)
    )
  })

  # 3. Pivot to Wide Format (Year | Var1 | Var2)
  table_data_wide <- correction_data %>%
    select(year, var_label, correction) %>%
    pivot_wider(names_from = var_label, values_from = correction)

  # 4. Generate the Consolidated GT Table
  tbl_trend_combined <- table_data_wide %>%
    gt() %>%
    # Label the Year column, others are auto-labeled by pivot_wider names
    cols_label(year = "År") %>%

    # Format all number columns (except Year)
    fmt_number(
      columns = -year,
      decimals = 3,
      dec_mark = ","
  ...
  # 5. Save ONE file
  gtsave(tbl_trend_combined, file = file.path(DIR_TAB, "trend_correction_combined.html"))
}
