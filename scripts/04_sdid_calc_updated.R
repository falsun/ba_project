...
sdid_placebo_2018_estimate <- synthdid_placebo(sdid_estimate, treated.fraction = 0.5)
...
# --- NEW: Placebo 2018 Model Bootstrap Calls ---
message("Calculating 2018 placebo model statistics (Bootstrap Call 9)...")
placebo_2018_se <- sqrt(vcov(sdid_placebo_2018_estimate, method = SE_METHOD)) # <<< CALL 9

message("Generating raw 2018 placebo plot (Bootstrap Call 10)...")
placebo_2018_plot_raw <- get_raw_sdid_plot(sdid_placebo_2018_estimate) # <<< CALL 10

message("Generating raw 2018 placebo overlay plot (Bootstrap Call 11)...")
placebo_2018_overlay_plot_raw <- get_raw_sdid_plot(sdid_placebo_2018_estimate, overlay = TRUE) # <<< CALL 11

message("Generating raw 2018 placebo units dot plot (Bootstrap Call 12)...")
placebo_2018_units_dot_plot_raw <- synthdid_units_plot(sdid_placebo_2018_estimate, se.method = SE_METHOD) # <<< CALL 12
...
saveRDS(sdid_placebo_2018_estimate, file = file.path(DIR_OUT, "sdid_placebo_2018_estimate.rds")) # <<< NEW
...
saveRDS(placebo_2018_se, file = file.path(DIR_OUT, "placebo_2018_se.rds")) # <<< NEW
...
saveRDS(placebo_2018_plot_raw, file = file.path(DIR_OUT, "placebo_2018_plot_raw.rds")) # <<< NEW
saveRDS(placebo_2018_overlay_plot_raw, file = file.path(DIR_OUT, "placebo_2018_overlay_plot_raw.rds")) # <<< NEW
saveRDS(placebo_2018_units_dot_plot_raw, file = file.path(DIR_OUT, "placebo_2018_units_dot_plot_raw.rds")) # <<< NEW
