...
START_YEAR <- 2013 # Russian annexation of Crimea and NATO Wales Summit
...
### 4.2.1. Military spending in constant US$ ----
sipri_usd_clean <- read_excel(SIPRI_MILEX, sheet = "Constant (2023) US$", skip = 5) %>%
  ...
  mutate(
    ...
    milex_usd = as.numeric(milex_usd) * 1000000
  ) %>%
...
### 4.2.2. Military spending as a share of GDP ----
...
### 4.2.3. Military spending as a share of government spending ----

sipri_gov_clean <- read_excel(SIPRI_MILEX, sheet = "Share of Govt. spending", skip = 7) %>%
  clean_names() %>%
  pivot_longer(
    cols            = starts_with("x"),
    names_to        = "year",
  ...
  mutate(
    iso3c = countrycode(country, origin = "country.name", destination = "iso3c"),
    milex_gov = as.numeric(milex_gov) * 100
  ) %>%
  filter(iso3c %in% SAMPLE_ISO3C, between(year, START_YEAR, END_YEAR)) %>%
  select(iso3c, year, milex_gov)

clean_panel <- merge_and_validate(clean_panel, sipri_gov_clean, c("iso3c", "year"), "SIPRI")
